...
func Test_should_implement_GetEpisodeInPort(t *testing.T) {
	...
	assert.Implements(t, (*onGetEpisode.GetEpisodePort)(nil), getEpisodeService)
}

func Test_should_throw_error_if_show_does_not_exist_on_get_episode(t *testing.T) {
	defer initAdapter()
	command := &onGetEpisode.GetEpisodeCommand{
...
func Test_should_propagate_errors_from_adapter_on_get(t *testing.T) {
	...
	foundEpisode, err := getEpisodeService.GetEpisode(&onGetEpisode.GetEpisodeCommand{EpisodeId: "id-with-error", ShowId: "some-show-id"})
...
func Test_should_return_not_found_if_episode_was_not_found_on_get(t *testing.T) {
	...
	foundShow, err := getEpisodeService.GetEpisode(&onGetEpisode.GetEpisodeCommand{EpisodeId: "id-with-error", ShowId: "some-show-id"})

	assert.Nil(t, foundShow)
	assert.NotNil(t, err)
	assert.Equal(t, domainError.NewEpisodeNotFoundError("id-with-error"), err)
	assert.Equal(t, 1, mockSaveAndGetEpisodeAdapter.calledGet)
}
...
func Test_retrieve_episode_from_repository_on_get(t *testing.T) {
	...
	expectedEpisodeResponse := &onGetEpisode.GetEpisodeResponse{
		Id:     "some-id",
		ShowId: "some-show-id",
		Title:  "some title",
	}
	...
	foundEpisode, err := getEpisodeService.GetEpisode(&onGetEpisode.GetEpisodeCommand{EpisodeId: "some-id", ShowId: "some-show-id"})
